  ...
  export class DrawingBoardComponent implements AfterViewInit {
    ...
    draw(e: MouseEvent) {
      if (this.isDragging && this.selectedShape) {
        ...
        if (currentShape.type === 'pen' || currentShape.type === 'eraser') {
          ...
          this.ctx.stroke();
        } else if (currentShape.type === 'startEvent' || currentShape.type === 'endEvent' ) {
    ...
    private saveText() {
      if (this.textInput && this.textInput.value.trim() !== '') {
        ...
        if (this.textInput.parentElement) {
          this.renderer.removeChild(this.canvasRef.nativeElement.parentElement, this.textInput);
        }
        this.textInput = null;
        this.redrawCanvas();
      }
    ...
    drawText() {
      if (this.textShape) {
        ...
        if (this.textShape.font) { // Perform null check for font
          this.ctx.font = this.textShape.font;
        }
        if (this.textShape.text) { // Perform null check for text
          this.ctx.fillText(this.textShape.text, this.textShape.startX, this.textShape.startY);
        }
      }
    }
    ...
    private redrawCanvas() {
      ...
      for (const shape of this.shapes) {
        ...
        this.ctx.setLineDash(shape.lineStyle === 'dashed' ? [5, 5] : shape.lineStyle === 'dotted' ? [2, 2] : []);
        ...
        switch (shape.type) {
          ...
          case 'eraser':
            ...
            case 'startEvent': {
              ...
              this.ctx.arc(shape.startX, shape.startY, radius * 0.6, 0, 2 * Math.PI);
          ...
          case 'gateway': {
            ...
            this.ctx.lineTo(shape.startX + shape.width!, shape.startY + shape.height! / 2);
            ...
            case 'task':
              this.ctx.beginPath();
                ...
                this.ctx.roundRect(shape.startX, shape.startY, shape.width!, shape.height!, rectRadius); // Rounded rectangle
                break;
    ...
    private getShapeAtPosition(pos: { x: number; y: number }): Shape | null {
      for (let i = this.shapes.length - 1; i >= 0; i--) {
        ...
        if (shape.type === 'rectangle' && this.isPointInRect(pos, shape)) {
          ...
          const distanceToCenter = Math.sqrt((pos.x - shape.startX) ** 2 + (pos.y - shape.startY) ** 2);
    ...
    private isPointInEventShape(pos: { x: number; y: number }, shape: Shape): boolean {
      ...
      return outerDistance <= outerRadius * outerRadius && innerDistance >= innerRadius * innerRadius;
    }
    private isPointInCircle(pos: { x: number; y: number }, shape: Shape): boolean {
    ...
    private isPointOnLine(pos: { x: number; y: number }, shape: Shape): boolean {
      ...
      const d1 = Math.sqrt(Math.pow(pos.x - startX, 2) + Math.pow(pos.y - startY, 2));
      const d2 = Math.sqrt(Math.pow(pos.x - endX!, 2) + Math.pow(pos.y - endY!, 2));
